...
class RController extends CController
{
	...
	*/
	...
	{
		if( $message===null )
			$this->redirect(Yii::app()->getBaseUrl(true));//$message = Rights::t('core', 'You are not authorized to perform this action.');

		$user = Yii::app()->getUser();
		if( $user->isGuest===true )
			$user->loginRequired();
		else
			$this->redirect(Yii::app()->getBaseUrl(true));//throw new CHttpException(403, $message);
	}
}
