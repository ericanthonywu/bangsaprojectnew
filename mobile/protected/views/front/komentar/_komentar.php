<?php 
	...
	$sql = "SELECT comment_email, comment_content, comment_status, comment_date, username, lastname, firstname FROM comments as a INNER JOIN users as b ON a.comment_user_id=b.id INNER JOIN profiles as c ON a.comment_user_id=c.user_id WHERE comment_news_id = :urlid AND comment_status = 1";
...
<div class="comment-berita">
	...
	<div class="post-comment">
		...
		<form action="<?php echo Yii::app()->createUrl('komentar/komentar', array('id'=>$urlID)); ?>" method="POST">
			<div class="photo-current-user mb1">
			<?php	if($zxzc['filename']):?>
						<img src="<?php echo "http://www.bangsaonline.com/images/uploads/user/".$zxzc['filename']; ?>" />
			<?php	else: ?>
						<img src="<?php echo "http://www.bangsaonline.com/images/uploads/user/default-user.jpg"; ?>" />
			<?php	endif; ?>
			</div>
			<div class="textarea-post-comment">
				<div class="mb1">
					<textarea name="Komentar[text]" class="form-control"></textarea>
				</div>
				<div>
					<input type="submit" value="KIRIM KOMENTAR" class="btn">
				</div>
			</div>
		</form>
		<div class="clearfix"></div>
	...
	<?php } ?>
	<div class="list-comment mt-mobile2">
		<ul class="comment-ok-loaded">
				<?php 
					$newsid = $_GET['id'];
					$sqllqlql = "SELECT c.*,u.*,p.`firstname`, p.`lastname` FROM comments c INNER JOIN users u ON (c.`comment_user_id`=u.`id`) INNER JOIN profiles p ON c.`comment_user_id`=p.`user_id` WHERE c.`comment_news_id` = '$newsid' AND comment_status = 1 LIMIT 5";
			...
			<li>
				...
				<?php 	$userID = $querys['comment_user_id'];
						...
						if($img['filename']): ?>
							<img src="<?php echo "http://www.bangsaonline.com/images/uploads/user/".$img['filename']; ?>" />
				<?php 	else:?>
						<img src="<?php echo "http://www.bangsaonline.com/images/uploads/user/default-user.jpg"; ?>" />
				<?php 	endif;?>
				</div>
