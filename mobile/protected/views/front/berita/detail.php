<?php
	...
	$berita_thumb_fb =Yii::app()->cache->get('berita_thumbD_fb_'.$idnews);
	if($berita_thumb_fb===false)
	{
		...
		Yii::app()->cache->set('berita_thumbD_fb_'.$idnews, $datathumb, 3600);
	}else{
		$datathumb = $berita_thumb_fb;
	}
	...
	$url 			= Yii::app()->getBaseUrl(true)."/berita/".$slugid;
...
?>
<div class="row bo-main blocks_custom_detail">
	<div class="span8 mb1 mt1"> 
		...
		<div class="block-detail-berita mt1">
			<div class="title-detail-berita">
				<h1><?php echo $data['news_title']; ?></h1>
			</div>
			<div class="date-detail-berita mb1"><?php echo get_time($data['news_date'],"dd MMMM yyyy | HH:MM","yes"); ?></div>
			<div class="att-detail-berita custom">
				<div class="row custom">
					...
					<div class="col-md-6 col-sm-6">
						<div class="att-sharing-berita" style="float:left;">
			...
			<?php 
			$berita_d_foto_caches =Yii::app()->cache->get('berita_dtfoto_'.$idnews);
			if($berita_d_foto_caches===false)
			{
				...
				Yii::app()->cache->set('berita_dtfoto_'.$idnews, $dataimage, 3600);
			}else{
				$dataimage = $berita_d_foto_caches;
			}
			?>
			<?php if(!empty($dataimage['filename'])){ ?>
				<div class="image-detail-berita mb1">
					<?php echo get_image($dataimage['filename'],"berita",'700',$dataimage['caption_title']); ?>
				</div>
				<?php if(!empty($dataimage['caption_desc'])){ ?>
					<div class="descimage-detail-berita mb1">
			...
			<div class="content-detail-berita">
				...
				<?php 	else:
							$sqliklant1 = "SELECT mio.object_id,mio.position,mi.id,mi.is_external,mi.link_iklan, mi.keterangan_iklan, mio.style FROM module_iklan_options mio INNER JOIN module_iklan mi ON(mio.object_id = mi.id) WHERE mio.position = 'mobile-detail-berita-tengah'";
							$dataiklant1		= Yii::app()->db->createCommand($sqliklant1)->queryRow(); 	
			...
			<!-- Iklan Bawah Detail Berita -->
			<?php $mio = Yii::app()->db->createCommand("SELECT style FROM module_iklan_options WHERE position = 'mobile-detail-berita-bawah'")->queryRow();?>
			<div class="iklan">
			<?php 	if($mio['style'] == 0):
						$sqliklant1 = "SELECT mio.object_id,mio.position,mi.id,mi.is_external,mi.link_iklan, mi.keterangan_iklan, mio.style, f.object_id,f.module_id,f.filename FROM module_iklan_options mio INNER JOIN module_iklan mi ON(mio.object_id = mi.id) INNER JOIN files f ON(mi.id=f.object_id) WHERE mio.position = 'mobile-detail-berita-bawah' AND f.module_id ='2' ";
						$dataiklant1		= Yii::app()->db->createCommand($sqliklant1)->queryRow(); 	
						if($dataiklant1['is_external'] =='1'){ $link = $dataiklant1['link_iklan']; }else{ $link = get_link($dataiklant1['id'],"iklan"); }  ?>
						...
						<?php endif;?>
			<?php 	else:
						$sqliklant1 = "SELECT mio.object_id,mio.position,mi.id,mi.is_external,mi.link_iklan, mi.keterangan_iklan, mio.style FROM module_iklan_options mio INNER JOIN module_iklan mi ON(mio.object_id = mi.id) WHERE mio.position = 'mobile-detail-berita-bawah'";
						$dataiklant1		= Yii::app()->db->createCommand($sqliklant1)->queryRow(); 	
						echo $dataiklant1['keterangan_iklan'];
					endif;?>
			</div>
			...
			<!-- Iklan Bawah Detail Berita 2 -->
			<?php $mio2 = Yii::app()->db->createCommand("SELECT style FROM module_iklan_options WHERE position = 'mobile-detail-berita-bawah2'")->queryRow();?>
			<div class="iklan">
			<?php 	if($mio2['style'] == 0):
						$sqliklant11 = "SELECT mio.object_id,mio.position,mi.id,mi.is_external,mi.link_iklan, mi.keterangan_iklan, mio.style, f.object_id,f.module_id,f.filename FROM module_iklan_options mio INNER JOIN module_iklan mi ON(mio.object_id = mi.id) INNER JOIN files f ON(mi.id=f.object_id) WHERE mio.position = 'mobile-detail-berita-bawah2' AND f.module_id ='2' ";
						$dataiklant11		= Yii::app()->db->createCommand($sqliklant11)->queryRow(); 	
						if($dataiklant11['is_external'] =='1'){ $link = $dataiklant11['link_iklan']; }else{ $link = get_link($dataiklant11['id'],"iklan"); }  ?>
						<?php if(!empty($dataiklant11['filename'])): ?>
							<a href="<?php echo $link; ?>"><?php echo get_image($dataiklant11['filename'],'iklan',null); ?></a>
						<?php endif;?>
			<?php 	else:
						$sqliklant11 = "SELECT mio.object_id,mio.position,mi.id,mi.is_external,mi.link_iklan, mi.keterangan_iklan, mio.style FROM module_iklan_options mio INNER JOIN module_iklan mi ON(mio.object_id = mi.id) WHERE mio.position = 'mobile-detail-berita-bawah2'";
						$dataiklant11		= Yii::app()->db->createCommand($sqliklant11)->queryRow(); 	
						echo $dataiklant11['keterangan_iklan'];
					endif;?>
			</div>
			<!-- Iklan Detail Berita Bawah 2 End -->
			...
			<div class="att-detail-berita">
				...
				<div class="tag-berita">
					<ul>
						<li class="first"><i class="fa fa-tags"></i> &nbsp;Tag: </li>
...
, CClientScript::POS_HEAD);?>
						...
						<?php 	foreach($datarelated as $drelated){
									...
									if($drelated['news_type'] == 1):?>
										<li>
											...
											<?php if(!empty($drelated['filename'])){ ?>
												<?php echo get_image($drelated['filename'],"berita",'100x100',$drelated['caption_title']); ?>
											<?php } ?>
											</a>
											<div class="title-newstab"><a href="<?php echo get_link_berita("berita",$drelated['news_id'],$drelated['news_slug']); ?>"><?php echo $drelated['news_title']; ?></a></div>
											<div class="clearfix"></div>
										</li>
						<?php 		elseif($drelated['news_type'] == 2):?>
										<li>
											<a href="<?php echo get_link_berita("beritafoto",$drelated['news_id'],$drelated['news_slug']); ?>">
											<?php if(!empty($drelated['filename'])){ ?>
												<?php echo get_image($drelated['filename'],"berita",'100x100',$drelated['caption_title']); ?>
											<?php } ?>
											</a>
											<div class="title-newstab"><a href="<?php echo get_link_berita("beritafoto",$drelated['news_id'],$drelated['news_slug']); ?>"><?php echo $drelated['news_title']; ?></a></div>
											<div class="clearfix"></div>
										</li>
						<?php 		elseif($drelated['news_type'] == 3):?>
										<li>
											<a href="<?php echo get_link_berita("beritavideo",$drelated['news_id'],$drelated['news_slug']); ?>">
											<?php if(!empty($drelated['filename'])){ ?>
												<?php echo get_image($drelated['filename'],"berita",'100x100',$drelated['caption_title']); ?>
											<?php } ?>
											</a>
											<div class="title-newstab"><a href="<?php echo get_link_berita("beritavideo",$drelated['news_id'],$drelated['news_slug']); ?>"><?php echo $drelated['news_title']; ?></a></div>
											<div class="clearfix"></div>
										</li>
						<?php		endif;
								} ?>
		...
		<?php echo $this->renderPartial('//site/widget-beritapopuler'); ?>
	</div>
</div>
	<div class="decoration"></div>
	<!-- SIDEBAR -->
	<div class="span4 mb1 mt2">
		<?php echo $this->renderPartial('//site/sidebar'); ?>
	</div>
