...
<?php 
	...
	<?php 	if($mio['style'] == 0):
				$sqliklant1 = "SELECT mi.id,mi.is_external,mi.link_iklan,f.filename FROM module_iklan_options mio INNER JOIN module_iklan mi ON(mio.object_id = mi.id) INNER JOIN files f ON(mi.id=f.object_id) WHERE mio.position = 'samping-topik-1' AND f.module_id ='2' ";
	...
	<?php 	else:
				$sqliklant1 = "SELECT mi.keterangan_iklan FROM module_iklan_options mio INNER JOIN module_iklan mi ON(mio.object_id = mi.id) WHERE mio.position = 'samping-topik-1'";
				$dataiklant1		= Yii::app()->db->createCommand($sqliklant1)->queryRow(); 	
...
<?php
	...
	<?php 	if($mio['style'] == 0):
				$sqliklant2 = "SELECT mi.id,mi.is_external,mi.link_iklan,f.filename FROM module_iklan_options mio INNER JOIN module_iklan mi ON(mio.object_id = mi.id) INNER JOIN files f ON(mi.id=f.object_id) WHERE mio.position = 'samping-topik-2' AND f.module_id ='2' ";
	...
	<?php 	else:
				$sqliklant2 = "SELECT mi.keterangan_iklan FROM module_iklan_options mio INNER JOIN module_iklan mi ON(mio.object_id = mi.id) WHERE mio.position = 'samping-topik-2'";
				$dataiklant2		= Yii::app()->db->createCommand($sqliklant2)->queryRow(); 	
