...
<?php 
	$dynamic_widget = "SELECT widget_id,widget_name,widget_type,object_id,widget_style FROM widget WHERE widget_name !='root' AND widget_group_id='4' ORDER BY lft ASC";
...
<?php }elseif($dwidget['widget_type'] == "category"){ ?>
	<?php
		...
		$sideCat_m_cache_{$key1} =Yii::app()->cache->get('sidecat_mnews_'.$key1);
		if($sideCat_m_cache_{$key1}===false)
		{
		// sql select category
		$thiscat 		= $dwidget['object_id'];
		...
		Yii::app()->cache->set('sidecat_mnews_'.$key1, $dataRes_sideCat, 1800);
		}else{
			$dataRes_sideCat = $sideCat_m_cache_{$key1};
		}		
	?>

	<?php if($dwidget['widget_style'] == "1" || $dwidget['widget_style'] == "0"){ ?>
	<!-- Widget Category Style 1 , Big Photo -->
	<div class="block-berita-3 mt1 mb1 customs_home_bottom" id="widget-<?php echo $dwidget['widget_name']; ?>">
			...
			?>
				...
				<div class="image-berita img-large">
					<a href="<?php echo get_link_berita("berita",$dataRes_sideCat['lastnewscat']['news_id'],$dataRes_sideCat['lastnewscat']['news_slug']); ?>">
						<?php echo get_image($dataRes_sideCat['dataimgnews']['filename'],'berita','340x170',$dataRes_sideCat['lastnewscat']['news_slug']); ?>
					</a>
				</div>
				<?php } ?>
			<?php }else{ ?>
	...
	<?php }else{ ?>
		...
		<div class="block-berita-4 mt1 mb1 customs_home_bottom">
