...
class XUpload extends CJuiInputWidget {
    ...
    public function run() {
        ...
        Yii::app() -> clientScript -> registerScript(__CLASS__ . '#' . $this -> htmlOptions['id'], "jQuery('#{$this->htmlOptions['id']}').fileupload({$options}).bind('fileuploadprocessfail', function (e, data) {
    ...
    public function publishAssets() {
        ...
        if (is_dir($assets)) {
            ...
            $messages = CJavaScript::encode(array(
                'fileupload' => array(
                    'errors' => array(
                        "maxFileSize" => $this->t('Ukuran file terlalu besar'),
                        "minFileSize" => $this->t('Ukuran file terlalu kecil'),
                        "acceptFileTypes" => $this->t('Format file tidak diperbolehkan'),
                        "maxNumberOfFiles" => $this->t('Jumlah file melebihi batas'),
                        "uploadedBytes" => $this->t('Ukuran file terlalu besar'),
                        "emptyResult" => $this->t('Hasil upload kosong'),
                    ),
                    'error' => $this->t('Error'),
                    'start' => $this->t('Upload'),
                    'cancel' => $this->t('Batal'),
                    'destroy' => $this->t('Hapus'),
                ),
            ));
