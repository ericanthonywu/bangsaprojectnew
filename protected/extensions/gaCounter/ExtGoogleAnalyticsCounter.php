...
*/
class ExtGoogleAnalyticsCounter extends CWidget
{
    public $ga_email;
    public $ga_password;
    public $ga_profile_id;
    public $strTotalVisits = "";
    public $strDayVisits = "";
    public $lastMonthChart = false;
    public $lastYearChart = false;
    public $customDateChart = false;
    public $startDate = "";
    public $endDate = "";
    public $typeChart = "month";
    public $width = 600;
    public $height = 300;
    public $title = "";
    ...
    public function init()
    {
        ...
        $this->ga_profile_id = Yii::app()->params['ga_profile_id'];
    }

    public function run()
    {
        if ($this->strTotalVisits !== "")
            $this->GetTotalVisits();
        if ($this->strDayVisits !== "")
            $this->GetTodayVisits();
        if ($this->lastMonthChart === true) {
            $this->startDate = date('Y-m-d', strtotime('-1 month -1 day'));
            $this->endDate = date("Y-m-d", strtotime('-1 day'));
            echo "<h4>" . $this->title . " " . $this->startDate . " - " . $this->endDate . "</h4>";
        }
        if ($this->lastYearChart === true) {
            $this->startDate = date('Y-m-d', strtotime('-1 year'));
            $this->endDate = date("Y-m-d");
            echo "<h4>" . $this->title . " " . $this->startDate . " " . $this->endDate . "</h4>";
        }
        if ($this->customDateChart === true) {
            echo "<h4>" . $this->title . " " . $this->startDate . " " . $this->endDate . "</h4>";
        }
        if ($this->startDate !== "" && $this->endDate !== "") {
            if ($this->typeChart == "month")
                $this->GetChartPerMonth($this->startDate, $this->endDate);
            else
                $this->GetChartPerDay($this->startDate, $this->endDate);
        }
    }

    public function GetTotalVisits()
    {
        if (Yii::app()->user->hasState('gACounter_TotalVisits')) {
            $totalVisits = Yii::app()->user->getState('gACounter_TotalVisits');
        } else {
            $ga = new gapi($this->ga_email, $this->ga_password);
            $ga->requestReportData($this->ga_profile_id, array("browser"), array('visits'), null, null, "2005-01-01", date("Y-m-d"));
            $totalVisits = $ga->getVisits();
            Yii::app()->user->setState('gACounter_TotalVisits', $totalVisits);
        }
        echo '<div style="background:#85c744;border-radius: 9px;color: #fff;display: inline-block;font-size: 11px;font-weight: 600;line-height: 10px;min-width: 10px;padding: 4px 6px;text-align: center;vertical-align: baseline;white-space: nowrap;margin-bottom:5px;">' . $this->strTotalVisits . ': ' . $totalVisits . '</div><br>';
    }

    public function GetTodayVisits()
    {
        if (Yii::app()->user->hasState('gACounter_DayVisits')) {
            $dayVisits = Yii::app()->user->getState('gACounter_DayVisits');
        } else {
            $ga = new gapi($this->ga_email, $this->ga_password);
            $ga->requestReportData($this->ga_profile_id, array('day'), array('visits'), null, null, date("Y-m-d"), date("Y-m-d"));
            $dayVisits = $ga->getVisits();
            Yii::app()->user->setState('gACounter_DayVisits', $dayVisits);
        }
        echo '<div style="background:#85c744;border-radius: 9px;color: #fff;display: inline-block;font-size: 11px;font-weight: 600;line-height: 10px;min-width: 10px;padding: 4px 6px;text-align: center;vertical-align: baseline;white-space: nowrap;">' . $this->strDayVisits . ': ' . $dayVisits . '</div><br>';
    }

    public function GetChartPerDay($startDate, $endDate)
    {
        $ga = new gapi($this->ga_email, $this->ga_password);
        $ga->requestReportData($this->ga_profile_id, array('year', 'month', 'day', 'date'), array('visits'), 'date', null, $startDate, $endDate, 1, 10000);
        $arrDays = array();
        $arrVisits = array();
        foreach ($ga->getResults() as $result) {
            $arrDays[] = $result->getDay() . "-" . $result->getMonth() . "-" . $result->getYear();
            $arrVisits[] = $result->getVisits();
        }
        $this->ShowChart($arrDays, $arrVisits);
    }

    public function GetChartPerMonth($startDate, $endDate)
    {
        $ga = new gapi($this->ga_email, $this->ga_password);
        $ga->requestReportData($this->ga_profile_id, array('month', 'year'), array('visits'), 'year,ga:month', null, $startDate, $endDate, 1, 10000);
        $arrDays = array();
        $arrVisits = array();
        foreach ($ga->getResults() as $result) {
            $arrDays[] = $result->getMonth();
            $arrVisits[] = $result->getVisits();
        }
        $this->ShowChart($arrDays, $arrVisits);
    ...
    private function ShowChart($arrLabels, $arrData)
    {
        $this->widget('chartjs.widgets.ChLine',
            array(
                'width' => $this->width,
                'height' => $this->height,
                'htmlOptions' => array(),
