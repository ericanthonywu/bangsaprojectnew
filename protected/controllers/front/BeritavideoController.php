...
class BeritavideoController extends Controller
{
	...
	public function actionIndex()
	{
		...
		// first 
		$sqltwo = "SELECT COUNT(news_id) AS countid FROM view_latest_news WHERE news_type=3 AND news_status=1 ORDER BY news_id DESC LIMIT 1";		
		$dataReaderTwo	= Yii::app()->db->createCommand($sqltwo)->queryRow();
		$idtwo = $dataReaderTwo['countid'];
		...
		// all
		if($idtwo !== 1){
			$sql = "SELECT * FROM `view_latest_news` `b` LEFT JOIN `view_files` `fls` ON(fls.object_id=b.news_id) WHERE news_id NOT IN('$idfirst') AND news_type=3 AND news_status=1 AND fls.module_id = '1'  ORDER BY news_id DESC LIMIT $start,$limit";			 
		}else{
			$sql = "SELECT * FROM `view_latest_news` `b` LEFT JOIN `view_files` `fls` ON(fls.object_id=b.news_id) WHERE news_type=3 AND news_status=1 AND fls.module_id = '1' ORDER BY news_id DESC LIMIT $start,$limit";	
		}
