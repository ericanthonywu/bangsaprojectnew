...
class BlockmobileController extends RController
{
	/**
	...
	*/
	...
	public function actionAddSeparator(){
		if(Yii::app()->request->isAjaxRequest){
			$forbiddenRoot = Blockmobile::model()->findByPk(1);
			$block = new Blockmobile;
	...
	public function actionAjaxRemoveItem(){
		if(Yii::app()->request->isAjaxRequest){
			...
			//z = id
			$z = $_POST['removeItem'];
			$block = Blockmobile::model()->findByPk($z);
			if(count($block) > 0){
				$block->deleteNode();
				...
				if(!isset($y['d']))
					...
					if(isset($parentRootBefore)){
						if( ($y['f'] == "category") || ($y['f'] == "iklan") ){
							$blockNow = Blockmobile::model()->findByPk($y['b']);
							$parentRootNow = Blockmobile::model()->findByPk($parentRootBefore);
	...
	{	
		Yii::app()->clientScript->registerScriptFile(Yii::app()->baseUrl.'/js/gktgnrs.js', CClientScript::POS_END);
		$this->render('index');
	}
