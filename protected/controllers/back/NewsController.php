	...
	*/
		...
		if(isset($_POST['News']))
		{
			...
			if($model->save()){
				...
				if ( isset($_GET['News_page']) ) {
					$params_red['News_page'] = ((int) $_GET['News_page']);
					$this->redirect(Yii::app()->createUrl('/news/index', $params_red));
				}else{
					$this->redirect(Yii::app()->createUrl('news'));
				}
			}
	...
	public function actionUploades()
	{
		...
		|| $_FILES['file']['type'] == 'image/pjpeg')
		{
			...
			$filename = md5($_FILES['file']['name'].date('Y-m-d H:i:s')).'.jpg';
			$file = $path.$filename;
			...
			// displaying file
			$array = array(
				'filelink' => Yii::app()->getBaseUrl(true)."/images/uploads/berita/".$filename
			);
