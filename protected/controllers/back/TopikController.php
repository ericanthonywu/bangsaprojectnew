	...
	{	
		...
		if(isset($_POST['Topik'])){
			...
			if(!empty($model->object_id)){
				...
				$tagArr = array();
				$tagAsx = explode(',', $model->object_id);
				foreach($tagAsx as $tag){
					$sanitizeTag = $this->sanitizeThis($tag);
					$tags = Yii::app()->db->createCommand("SELECT tag_name FROM tag WHERE tag_name = '$tag' or tag_name = '$sanitizeTag'")->queryRow();
					if(!$tags){
						Yii::app()->db->createCommand("INSERT INTO tag(tag_name, tag_title) VALUES('$sanitizeTag', '$tag')")->query();
					}
				}
