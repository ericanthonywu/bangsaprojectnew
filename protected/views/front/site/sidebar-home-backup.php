...
<?php 
	$dynamic_widget = "SELECT widget_id,widget_name,widget_type,object_id,widget_style FROM widget WHERE widget_name !='root' AND widget_group_id='3' ORDER BY lft ASC";
...
<?php }elseif($dwidget['widget_type'] == "category"){ ?>
	...
	<?php
		...
		$sqlastnewscat 	= "SELECT b.news_id,b.news_content,b.news_title,b.news_date,b.news_slug,b.ayat,b.oleh FROM module_news b INNER JOIN category_relationship cr ON(cr.category_id='$thiscat') WHERE cr.post_id = b.news_id AND news_type='1' AND b.news_status =1 ORDER BY b.news_id DESC LIMIT 1";
	...
	<div class="block-berita-3 mt1 mb1">
		<div class="top-block-berita-3">
			<div class="title-cat-berita df_top_section">
				<h3>
					<a href="<?php echo get_link($slugcat['category_slug'],"kanal"); ?>"><?php echo $dwidget['widget_name']; ?></a>
				</h3>
			...
			<div class="image-berita img-large">
				<a href="<?php echo get_link_berita("berita",$lastnewscat['news_id'],$lastnewscat['news_slug']); ?>">
					<?php echo get_image($dataimgnews['filename'],'berita','340x170',$lastnewscat['news_slug']); ?>
				</a>
			</div>
			<?php } ?>
	...
	<?php }else{ ?>
		...
		<div class="block-berita-4 mb1 customs_home_bottom">
			<div class="top-block-berita-4">
				...
				?>
					<?php if(!empty($dataimgnews)){ ?>
						<div class="image-berita img-thumb padding-bottom-10">
							<a href="<?php echo get_link_berita("berita",$lastnewscat['news_id'],$lastnewscat['news_slug']); ?>">
								<?php echo get_image($dataimgnews['filename'],'berita','100x100',$lastnewscat['news_slug']); ?>
							</a>
						</div>
					<?php } ?>
				<?php }else{ ?>
...
<?php }elseif($dwidget['widget_type'] == "iklan"){
	...
	<?php 	if($dwidget['widget_style'] == "1"):
				$sqliklanside = "SELECT mi.id,mi.is_external,mi.link_iklan, mi.keterangan_iklan,f.filename FROM module_iklan mi INNER JOIN files f ON(f.object_id ='$idwidgiklan') WHERE mi.id = '$idwidgiklan' AND f.module_id ='2' ";
	...
	<?php 	else:
				$sqliklanside = "SELECT mi.keterangan_iklan FROM module_iklan mi WHERE mi.id = '$idwidgiklan'";
				$dataiklanside		= Yii::app()->db->createCommand($sqliklanside)->queryRow();
