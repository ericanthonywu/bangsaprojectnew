...
<div class="row bo-main">
	...
	<?php $this->renderPartial('iklan-atas-topik-khas'); ?>
	...
	<div class="sub-row">
		...
		<?php if($datab['block_type'] == "category"){ ?>
			<?php 
				...
				$datacat		= Yii::app()->db->createCommand($sqlselectcat)->queryRow();
					...
					// $sqlnewshimage = "SELECT 
					// `b`.`news_id`,`b`.`news_slug`,`b`.`news_title`,`b`.`news_date`,`cr`.*,`f`.`filename`
					// FROM `category_relationship` `cr` 
					// INNER JOIN `module_news` `b` ON(`cr`.`post_id` = `b`.`news_id`) 
					// INNER JOIN `files` `f` ON(`cr`.`post_id` = `f`.`object_id`) 
					// WHERE `cr`.`category_id`= '$thiscat' 
					// AND `f`.`module_id`=1 
					// AND `b`.`news_type`=1 
					// AND `b`.`news_status`=1 
					// ORDER BY `b`.`news_id` DESC LIMIT 1";

					$sqlnewshimage = "SELECT * FROM view_kanal_news b
					...
					// cache
					$latest_news_c_{$keysL} =Yii::app()->cache->get('latest_news_c_'.$keysL);
					if($latest_news_c_{$keysL}===false)
					{
						...
						Yii::app()->cache->set('latest_news_c_'.$keysL, $lastestnews, 300);
					}else{
		...
		<?php }elseif($datab['block_type'] == "iklan"){ ?>
			...
			?>
			<div class="block-iklan-2" style="padding-top:10px;text-align:center;">
				<div class="clearfix"></div>
			<?php 	if($datab['block_style'] == 1):
						$sqliklankontent = "SELECT f.filename,mi.is_external,mi.link_iklan,mi.id FROM module_iklan mi INNER JOIN files f ON(mi.id=f.object_id) WHERE f.module_id='2' AND mi.id='$iklanid' ";
