	...
	<div class="span8 mb1 mt1"> 
		...
		<div class="list-category-indeks">	
			...
			<?php 
				...
				$sqlcheck = "SELECT * FROM `news_indeks` WHERE category_id = '$thiscatid' AND DATE(news_date) LIKE '%$tahun-$bulan-$tanggal%' AND category_id LIKE '{$kanal}'";
				$datanewscheck	= Yii::app()->db->createCommand($sqlcheck)->queryAll();
			?>
				<?php if(!empty($datanewscheck)){ ?>
					<div class="category-item" id="">
						<div class="title-cagory-item"><a href="<?php echo get_link($dindeks['category_slug'],"kanal"); ?>"><?php echo $dindeks['category_name']; ?></a></div>
