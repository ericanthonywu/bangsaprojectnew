<?php
	...
	// Link Alternate for Google Mobile Test
	$mobileurl 		= 'http://m.bangsaonline.com/';
	$mobileurl2 	= "berita/".$slugid;
...
?>
<!-- <link rel="image_src" type="image/jpeg" href="<?php //echo $image; ?>" title="<?php //echo $title; ?>" /> -->
...
<div class="row bo-main">
	<div class="span8 mb1 mt1"> 
		...
		<div class="block-detail-berita">
			...
			{
				$sqlgetimage 	= "SELECT object_id,module_id,filename,caption_title,caption_desc FROM view_files WHERE object_id='$idnews' AND module_id='1' ";
			...
			<?php if(!empty($dataimage['filename'])){ ?>
				...
				<?php /* <!-- $datathumb['caption_title'] --> */ ?>
					<?php echo get_image($datathumb['filename'],"berita","700", $alt_news); ?>
				</div>
				<?php if(!empty($dataimage['caption_desc'])){ ?>
					<div class="descimage-detail-berita">
			...
			<?php 
			// iklan detail bawah berita 2
			$mio2 = Yii::app()->db->createCommand("SELECT style, object_id FROM module_iklan_options WHERE position = 'bawah-detail-berita2'")->queryRow();?>
			<?php if($mio2['object_id'] != 0):?>
			<div class="mt1">
			<?php 	if($mio2['style'] == 0):
						$sqliklant11 = "SELECT mio.object_id,mio.position,mi.id,mi.is_external,mi.link_iklan, mi.keterangan_iklan, mio.style, f.object_id,f.module_id,f.filename FROM module_iklan_options mio INNER JOIN module_iklan mi ON(mio.object_id = mi.id) INNER JOIN files f ON(mi.id=f.object_id) WHERE mio.position = 'bawah-detail-berita2' AND f.module_id ='2' ";
			...
			<?php 	else:
						$sqliklant11 = "SELECT mio.object_id,mio.position,mi.id,mi.is_external,mi.link_iklan, mi.keterangan_iklan, mio.style FROM module_iklan_options mio INNER JOIN module_iklan mi ON(mio.object_id = mi.id) WHERE mio.position = 'bawah-detail-berita2'";
						$dataiklant11		= Yii::app()->db->createCommand($sqliklant11)->queryRow(); 	
...
, CClientScript::POS_HEAD);?>
				...
				<div class="list-berita-terkait related_news_edit">
					<?php 
						...
						if(!empty($datatag)){
							...
							{
									$sqlrelatednews = "SELECT b.news_title, b.news_id, b.news_image, b.news_slug, b.news_type, b.caption_title, b.caption_desc, b.filename 
	...
	<!-- SIDEBAR -->
	<div class="span4 mb1" style="margin-top: 5px;">
		<?php $this->renderPartial('//site/widget-beritapopuler'); ?>
